    ...
    <div class="page-title-inner flex-row  medium-flex-wrap container">
        ...
        <div class="flex-col medium-text-center">
            <p class="woocommerce-result-count hide-for-medium">
                Hiển thị 1–12 của {{$all_Product->count()}} kết quả</p>
                ...
                <aside id="search-5" class="widget widget_search">
                    <form method="get" class="searchform" action="" role="search">
                        <div class="flex-row relative">
                            ...
                            <div class="flex-col">
                                <button type="submits" class="ux-search-submit submit-button secondary button icon mb-0" aria-label="Submit">
...
<script type='text/javascript'>
    ...
    $(".search-field").keyup(function(){
        ...
        $.ajax({
            type: "GET",
            url: "/san-pham/index.html?key_search="+$(this).val(),
            ...
            success: function (response) {
                ...
                if(response.length != 0){
                    ...
                    $.each(response, function(key, val) {
                        sz += `<div class="autocomplete-suggestion" data-index="`+val.PR_alias+`"><img class="search-image" src="{{url('resources/upload/product/')}}/`+val.PR_avatar+`">
                            <div class="search-name">`+val.PR_name+`</div>
                        </div>`;
                        console.log(sz);
                    });
                }
                ...
                if(sz != ''){
                    $('.autocomplete-suggestions').append(sz);
                }else{
                    $('.autocomplete-suggestions').empty();
                }
                $('.ux-search-submit').removeClass('loading');
            },
        });
        $('.autocomplete-suggestions').show();
    });
    $( "#autocomplete-suggestions" ).on( "mouseover", '.autocomplete-suggestion', function() {
        $(this).addClass('autocomplete-selected');
    });
    $( "#autocomplete-suggestions" ).on( "mouseout", '.autocomplete-suggestion', function() {
        $(this).removeClass('autocomplete-selected');
    });
    $( "#autocomplete-suggestions" ).on( "click", '.autocomplete-suggestion', function() {
        window.location.href = "/product/"+$(this).data('index')+"/detail.html";
    });
